...
import { startupCostTemplates } from '@/data/startupCostTemplates';
...
export default function StartupCostTab() {
  ...
  const getSummaryCardClassName = (metricName: string, defaultClasses: string): string =>
    getSummaryCardClasses(metricName, feedbackItems, defaultClasses, {
      critical: 'p-4 border-l-4 border-destructive bg-destructive bg-opacity-10',
      warning: 'p-4 border-l-4 border-yellow-400 bg-yellow-400 bg-opacity-10',
      info: 'p-4 border-l-4 border-blue-400 bg-blue-400 bg-opacity-10',
      good: 'p-4 border-l-4 border-green-400 bg-green-400 bg-opacity-10',
    });
  ...
  const applyTemplate = (type: string) => {
    const templateItems = startupCostTemplates[type] ?? items;
